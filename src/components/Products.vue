        ...
        <div class="row">
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-5">
                ...
                <select class="form-select py-2 shadow-sm text-capitalize mb-4" v-model="filterByCategory">
                    ...
                    <option :value="product" v-for="product in eliminateDuplicatedCategories($store.state.products)"
                        :key="product.id">
                        {{product}}
                    </option>
                </select>
                <!-- Sort by prices -->
                <select class="form-select py-2 shadow-sm mb-4" v-model="sortByPrice"
                    @change="sortProductsByPrice($store.state.products)">
                    <option value="" disabled>Filtar por Preço</option>
                    <option value="increasingOrder">Ordem Crescente</option>
                    <option value="decreasingOrder">Ordem Decrescente</option>
                </select>
                <!-- Sort by ratings -->
                <select class="form-select py-2 shadow-sm mb-4" v-model="sortByRating"
                    @change="sortProductsByRating($store.state.products)">
                    <option value="" disabled>Filtrar por Nota</option>
                    <option value="increasingOrder">Ordem Crescente</option>
                    <option value="decreasingOrder">Ordem Decrescente</option>
                </select>
            </div>
            <div class="col-xl-10 col-lg-9 col-md-8 col-sm-7">
                ...
                <div class="input-group mb-4 shadow-sm">
                    ...
                    <input type="text" class="form-control py-2" placeholder="Procurar por um produto..."
                        aria-label="Username" aria-describedby="basic-addon1" v-model="searchProduct">
                </div>
                <!-- Display Products -->
                <div class="d-flex flex-wrap justify-content-sm-between justify-content-center">
                    <div class="card mb-4 shadow-sm" v-for="(product,index) in filterProducts" :key="product.id">
                        <div class="card-img">
...
export default {
    ...
    methods: {
        discardProduct(index) {
            ...
            window.localStorage.setItem('Produtos', JSON.stringify(this.$store.state.products))
        },
